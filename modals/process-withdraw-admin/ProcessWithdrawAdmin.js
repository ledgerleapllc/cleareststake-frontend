...
class ProcessWithdrawAdmin extends Component {
  constructor(props) {
    super(props);
    this.state = {
      user: 0,
  ...
  submit = (e) => {
    ...
    if (parseInt(amount) > parseInt(selectedUser.balance)) {
      this.props.dispatch(
        showAlert(`This user has ${selectedUser.balance} tokens available`)
      );
      return;
    }
  ...
  render() {
    ...
    return (
      <div id="process-withdraw-admin-modal">
        <h3>Record Withdraw</h3>
        <label className="mt-4 d-block">Which user is withdrawing?</label>
        <select
          className="custom-form-control"
          value={user}
        ...
        <div id="process-withdraw-admin-modal__buttons">
...
export default connect(mapStateToProps)(withRouter(ProcessWithdrawAdmin));
